        ...
        if (statusRes == EFLASH_STATUS_SUCCESS)
        {
            ...
            if (statusRes == EFLASH_STATUS_SUCCESS)
            {
                ...
                if (statusRes == EFLASH_STATUS_SUCCESS)
                {
                    ...
                    /* Check error */
                    if (EFLASH_GetStatusReg() & EFLASH_SR1_ERAER_MSK)
                    {
...
EFLASH_STATUS EFLASH_PageProgram(uint32_t pageAddress, uint32_t *dataBuffer, uint32_t dataLength)
{
    ...
    if (statusRes == EFLASH_STATUS_SUCCESS)
    {
        ...
        if (statusRes == EFLASH_STATUS_SUCCESS)
        {
            ...
            /* Configure command and CMD_ST goes back to 0 */
            cmdValue = (dataLength << EFLASH_CTRL1_PROG_LENGTH_POS) | (uint32_t)EFLASH_CMD_PAGEPROGRAM;
            EFLASH_SetCtlReg1(cmdValue);
            ...
            if (statusRes == EFLASH_STATUS_SUCCESS)
            {
                ...
                if (statusRes == EFLASH_STATUS_SUCCESS)
                {
                    ...
                    /* Check error */
                    if (EFLASH_GetStatusReg() & EFLASH_SR1_PPADRERR_MSK)
                    {
    ...
    if (statusRes == EFLASH_STATUS_SUCCESS)
    {
        ...
        if (statusRes == EFLASH_STATUS_SUCCESS)
        {
            ...
            if (statusRes == EFLASH_STATUS_SUCCESS)
            {
                ...
                {
                    /* Check verify result */
                    if (EFLASH_GetStatusReg() & EFLASH_SR1_VRER_MSK)
                    {
 ...
 */
static EFLASH_STATUS EFLASH_OptionPageErase(uint32_t optPageAddress)
{
    EFLASH_STATUS statusRes = EFLASH_STATUS_SUCCESS;
    uint32_t cmdValue = 0x0;
    ...
    if (statusRes == EFLASH_STATUS_SUCCESS)
    {
        ...
        /* Configure command and CMD_ST goes back to 0 */
        cmdValue = EFLASH_CTRL1_OPT_CMD_EN_MSK | (uint32_t)EFLASH_CMD_PROTECTERASE;
        EFLASH_SetCtlReg1(cmdValue);
 ...
 */
static EFLASH_STATUS EFLASH_OptionPageProgram(uint32_t pageAddress, uint32_t *dataBuffer, uint32_t dataLength)
{
    EFLASH_STATUS statusRes = EFLASH_STATUS_SUCCESS;
    uint32_t cmdValue = 0x0;
    uint32_t dataCount = 0x0;
    ...
    if (statusRes == EFLASH_STATUS_SUCCESS)
    {
        ...
        /* Configure command and CMD_ST goes back to 0 */
        cmdValue = (dataLength << EFLASH_CTRL1_PROG_LENGTH_POS) | EFLASH_CTRL1_OPT_CMD_EN_MSK | (uint32_t)EFLASH_CMD_PROTECTROGRAM;
 ...
 * @brief Set write protect by page
 */
EFLASH_STATUS EFLASH_SetPageWriteProtect(uint32_t startPageNumber, uint32_t pageLength , uint8_t state)
{
    EFLASH_STATUS statusRes = EFLASH_STATUS_SUCCESS;
    uint32_t optBuffer[12] = {0};
    uint32_t i;

    if ((startPageNumber + pageLength) > (EFLASH_SIZE / EFLASH_PAGE_SIZE))
    {
        return EFLASH_STATUS_PARAMETER_ERROR;
    }
