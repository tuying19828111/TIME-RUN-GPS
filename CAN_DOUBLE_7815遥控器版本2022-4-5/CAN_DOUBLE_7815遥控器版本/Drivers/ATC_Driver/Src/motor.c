...
void PWM_ChangeMotorPhaseUseTrigMode(void)
{
    ...
    PWM2->SYNCONF |= PWM_SYNCONF_SYNCMODE_Msk | PWM_SYNCONF_CNTVSWSYNC_Msk | PWM_SYNCONF_PWMSVSWSYNC_Msk;  ////SW Trig MCVR and CHnV
}
/**
*
...
*
*/
void PWM_MotorOutput(MOTOR_PHASE positivePhase, MOTOR_PHASE nagetivePhase)
{
    uint8_t i = 0;
    PWM_MaskAllChannels();
    PWM_ClearAllPWMOutput();
    for (i = 0; i < (uint8_t)PHASE_NUM; i++)
    {
        if (i == (uint8_t)positivePhase)
        {
